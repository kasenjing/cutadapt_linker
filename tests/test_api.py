...
def test_command_line():
    ...
    stats = main(["-q", "10", "-o", 'out.1.txt', '-p', 'out.2.txt', datapath("1.txt"),  datapath("2.txt"), "--linker", '-b', 'ACGCGATATCTTATTGACT'])
...
def test_pipeline_linker_paired(tmp_path):
    # cutadapt -u 5 -U 7 -a GATCGGAAGA -q 0,15 -m 10:0
    #   --discard-untrimmed --info-file=info.txt
    #   -o ... -p ...
    #   paired.1.fastq paired.2.fastq
    # tmp_path = "."
    from cutadapt.pipeline import PairedEndPipeline, LinkerPairedEndPipeline
    from cutadapt.modifiers import UnconditionalCutter, QualityTrimmer, AdapterCutter, LinkerSingleAdapterCutter, ReverseComplementerLinkerSingle, ReverseComplementer, ReverseComplementerLinkerPaired
    from cutadapt.adapters import BackAdapter, AnywhereAdapter
    from cutadapt.files import OutputFiles, InputPaths, FileOpener

    trimmer = QualityTrimmer(cutoff_front=0, cutoff_back=15)
    adapter = AnywhereAdapter(
        sequence="ACGCGATATCTTATTGACT",
        max_errors=2,
        min_overlap=6,
    )
    adapter_cutter = LinkerSingleAdapterCutter([adapter], 1)
    modifiers = [
        (trimmer, copy.copy(trimmer)),
        ReverseComplementerLinkerPaired(adapter_cutter, None)
    ]
    sys.stderr.write("Pipeline2: "+"\n")
    sys.stderr.write(str(modifiers))
    pipeline = LinkerPairedEndPipeline(modifiers, "any")

    pipeline.minimum_length = (18, 18)
    pipeline.discard_untrimmed = True

    file_opener = FileOpener()
    inpaths = InputPaths(datapath("R1_M.fq"), datapath("R2_M.fq"))

    # inpaths = InputPaths(datapath("1.txt"), datapath("2.txt"))
    info_file = file_opener.xopen_or_none("info.txt", "wb")
    out, out2 = file_opener.xopen_pair(
         "out.1.fastq", "out.2.fastq", "wb"
    )
    outfiles = OutputFiles(
        info=info_file,
        out=out,
