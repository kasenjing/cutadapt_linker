...
class LinkerSingleAdapterCutter(PairedEndModifier):
    """
    Added by Kasen Jing, 2024/01/08

    Find one adapter and return the part in front of linker and the part after the linker.
    Arguments:
        adapters: Adapters to be searched
        times: Repeat the search this number of times.
        action: What to do with a found adapter.
            - "trim": remove the adapter and return the down-stream and upstream sequence.
            - "mask": Replace the part of the sequence that would have been removed with "N" bases
            - "lowercase" Convert the part of the sequence that would have been removed to lowercase
            - "retain": Like "trim", but leave the adapter sequence itself in the read
    """
    ...
    def __call__(self, read1, read2, info1: ModificationInfo, info2: ModificationInfo):
        trimmed_read, trimmed_read_right, matches = self.match_and_trim(read1)
        if matches:
            self.with_adapters += 1
            for match in matches:
                self.adapter_statistics[match.adapter].add_match(match)
        info1.matches.extend(matches)  # TODO extend or overwrite?
        info2.matches.extend(matches)
        return trimmed_read, trimmed_read_right

    def match_and_trim(self, read):
        
        sys.stderr.write("Time: "+str(self.times))
        matches = []
        if self.action == "lowercase":  # TODO this should not be needed
            read.sequence = read.sequence.upper()
        trimmed_read = read
        trimmed_read_right = read
        for _ in range(self.times):
            match = self.adapters.match_to(trimmed_read.sequence)
            if match is None:
                # if nothing found, attempt no further rounds
        ...
        sys.stderr.write(trimmed_read)
        sys.stderr.write(trimmed_read_right)
        if not matches:
            return trimmed_read, trimmed_read_right, []

        if self.action == "trim":
            trimmed_read, trimmed_read_right = matches[-1].split_from_adapter(trimmed_read)
            # read is already trimmed, nothing to do
            pass
        elif self.action == "retain":
            trimmed_read = self.trim_but_retain_adapter(read, matches)
        ...
        return trimmed_read, trimmed_read_right, matches
    ...
    def _match_and_trim_once_action_trim(self, read):
        ...
        match = self.adapters.match_to(read.sequence)
        if match is not None:
            trimmed_read, trimmed_read_right = match.split_from_adapter(read)
            return trimmed_read, trimmed_read_right, [match] 
        else:
            return read, None, []

class ReverseComplementerLinkerSingle(PairedEndModifier):

    '''
        split reads from adapter
    '''

    def __init__(self, adapter_cutter: LinkerSingleAdapterCutter, rc_suffix: Optional[str] = " rc"):
        """
        rc_suffix -- suffix to add to the read name if sequence was reverse-complemented
        """
        self.adapter_cutter = adapter_cutter
    ...
    def __repr__(self):
        return f"ReverseComplementerLinkerSingle(adapter_cutter={self.adapter_cutter})"

    def __call__(self, read1: SequenceRecord, read2: SequenceRecord, info1: ModificationInfo, info2: ModificationInfo):
        reverse_read = read1.reverse_complement()
    
        forward_trimmed_read, forward_trimmed_read_right, forward_matches = self.adapter_cutter._match_and_trim_once_action_trim(read1)
        reverse_trimmed_read, reverse_trimmed_read_right, reverse_matches = self.adapter_cutter._match_and_trim_once_action_trim(
            reverse_read
        )
        ...
        if use_reverse_complement:
            self.reverse_complemented += 1
            assert reverse_matches
            trimmed_read, trimmed_read_right, matches = reverse_trimmed_read, reverse_trimmed_read_right,  reverse_matches
            info1.is_rc = True
            info2.is_rc = True  
            if self._suffix:
                trimmed_read.name += self._suffix
                trimmed_read_right.name += self._suffix
        else:
            info1.is_rc = False
            info2.is_rc = False
            trimmed_read, trimmed_read_right, matches = forward_trimmed_read, forward_trimmed_read_right, forward_matches
...
class ReverseComplementerLinkerPaired(PairedEndModifier):
    """Trim adapters from a read pair or its reverse complement (R1/R2 swapped)"""

    def __init__(
        self,
        adapter_cutter1: Optional[LinkerSingleAdapterCutter],
        adapter_cutter2: Optional[LinkerSingleAdapterCutter],
        rc_suffix: Optional[str] = " rc",
    ):
        """
        rc_suffix -- suffix to add to the read name if sequence was reverse-complemented
    ...
    def __repr__(self):
        return (
            "ReverseComplementerLinkerPaired("
            f"adapter_cutter1={self.adapter_cutter1}, adapter_cutter2={self.adapter_cutter2})"
        )
    ...
    def __call__(
        ...
        ## first try to concatenate read 1 and read 2
        crossed = False
        r1_reverse_read = r1.reverse_complement()
        r2_reverse_read = r2.reverse_complement()

        # Find adapter in Read 1, using adapter1 
        r1_forward_trimmed_read, r1_forward_trimmed_read_right, r1_matches = self.adapter_cutter1.match_and_trim(r1)
        r1_reverse_trimmed_read, r1_reverse_trimmed_read_right, r1_reverse_matches = self.adapter_cutter1.match_and_trim(r1_reverse_read)
        r2_forward_trimmed_read, r2_forward_trimmed_read_right, r2_matches = self.adapter_cutter1.match_and_trim(r2)
        r2_reverse_trimmed_read, r2_reverse_trimmed_read_right, r2_reverse_matches = self.adapter_cutter1.match_and_trim(r2_reverse_read)
        ...
        r1_real_matches = []
        r2_real_matches = []
        # decide if two half linker
        whole_adapter_min_overlap = self.adapter_cutter1.adapters._adapters[0].min_overlap*2
        linker_type = ["none", "none"]
        if r1_matches:
            match = r1_matches[0]
            r1_real_matches.append(match)
            ### forward linker find at read start
            if match.rstop < whole_adapter_min_overlap:
                linker_type[0] = 'header'
                if cpu_like:
                    r1_trimmed = r1_forward_trimmed_read_right
                    crossed = True
            ### forward linker find at read 1 mid
            else:
                if match.rstart > (len(r1.sequence)-whole_adapter_min_overlap):
                    linker_type[0] = 'tail'
                else:
                    linker_type = ['tail', 'tail']
                r1_trimmed = r1_forward_trimmed_read

        if r2_matches:
            match = r2_matches[0]
            r2_real_matches.append(match)
            ### forward partial linker find at read 2 start
            if match.rstop < whole_adapter_min_overlap:
                linker_type[1] = 'header'
                if cpu_like:
                    r2_trimmed = r2_forward_trimmed_read_right
            else:
                if match.rstart > (len(r2.sequence)-whole_adapter_min_overlap):
                    linker_type[1] = 'tail'
                else:
                    linker_type = ['tail', 'tail']
                r2_trimmed = r2_forward_trimmed_read
                crossed = True

        if r1_reverse_matches:
            match = r1_reverse_matches[0]
            r1_real_matches.append(match)
            if match.rstart > (len(r1.sequence)-whole_adapter_min_overlap):
                linker_type[0] = 'header'
                #### r1 reverse find partial linker require r2 have partial linker too
                if cpu_like:
                    r1_trimmed = r1_reverse_trimmed_read.reverse_complement()
            else:
                if match.rstop < whole_adapter_min_overlap:
                    linker_type[0] = 'tail'
                else:
                    linker_type = ['tail', 'tail']
                r1_trimmed = r1_reverse_trimmed_read_right.reverse_complement()
                crossed = True
        
        if r2_reverse_matches:
            match = r2_reverse_matches[0]
            r2_real_matches.append(match)
            if match.rstart > (len(r2.sequence)-whole_adapter_min_overlap):
                linker_type[1] = 'header'
                if cpu_like:
                    r2_trimmed = r2_reverse_trimmed_read.reverse_complement()
                    crossed = True
            else:
                if match.rstop < whole_adapter_min_overlap:
                    linker_type[1] = 'tail'
                    ...
                    linker_type = ['tail', 'tail']
                r2_trimmed = r2_reverse_trimmed_read_right.reverse_complement()

        if linker_type != ['tail', 'tail']:
            if cpu_like:
                if linker_type != ['header', 'header']:
                    r1_trimmed = r1
                    r2_trimmed = r2
                    r1_real_matches = []
                    r2_real_matches = []
        ...
        if r1_real_matches:
            ...
            for match in r1_real_matches:
                stats = self.adapter_cutter1.adapter_statistics[match.adapter]  # type: ignore
                stats.add_match(match)
                stats.reverse_complemented += bool(crossed)
            info1.matches.extend(r1_real_matches)  # TODO extend or overwrite?
            info2.matches.extend(r1_real_matches)
        if r2_real_matches:
            self.adapter_cutter1.with_adapters += 1  # type: ignore
            for match in r2_real_matches:
                stats = self.adapter_cutter1.adapter_statistics[match.adapter]  # type: ignore
                stats.add_match(match)
                stats.reverse_complemented += bool(crossed)
            info2.matches.extend(r2_real_matches)  # TODO extend or overwrite?
            info1.matches.extend(r2_real_matches)
        ...
        if crossed:
            ...
            info1 = info2
            info2 = tmpinfo
            tmp_trimmed = r1_trimmed
            r1_trimmed = r2_trimmed
            r2_trimmed = tmp_trimmed

            tmp_name = r1_trimmed.name
            r1_trimmed.name = r2_trimmed.name
            r2_trimmed.name = tmp_name
        return r1_trimmed, r2_trimmed
##### Changing Block End
    


#### Trying to concatenate reads, too slow.
# class ReverseComplementerLinkerPairedTmp(PairedEndModifier):
#     """Trim adapters from a read pair or its reverse complement (R1/R2 swapped)"""
...
#     def __repr__(self):
#         return (
#             "PairedReverseComplementerLinkerPaired("
#             f"adapter_cutter1={self.adapter_cutter1}, adapter_cutter2={self.adapter_cutter2})"
#         )
...
#         # print("r1_forward_trimmed: "+(r1_forward_trimmed_read.sequence if r1_forward_trimmed_read else "none"))
#         # print("r1_forward_trimmed_right: "+(r1_forward_trimmed_read_right.sequence if r1_forward_trimmed_read_right else "none"))
#         # print("r2_forward_trimmed: "+(r2_forward_trimmed_read.sequence if r2_forward_trimmed_read else "none"))
#         # print("r2_forward_trimmed_right: "+(r2_forward_trimmed_read_right.sequence if r2_forward_trimmed_read_right else "none"))
#         # print("r1_reverse_trimmed: "+(r1_reverse_trimmed_read.sequence if r1_reverse_trimmed_read else "none"))
#         # print("r1_reverse_trimmed_right: "+(r1_reverse_trimmed_read_right.sequence if r1_reverse_trimmed_read_right else "none"))
#         # print("r2_reverse_trimmed: "+(r2_reverse_trimmed_read.sequence if r2_reverse_trimmed_read else "none"))
#         # print("r2_reverse_trimmed_right: "+(r2_reverse_trimmed_read_right.sequence if r2_reverse_trimmed_read_right else "none"))


#         r1_score = sum(m.score for m in r1_matches) 
#         r1_reverse_score = sum(m.score for m in r1_reverse_matches) 
#         r2_score = sum(m.score for m in r2_matches) 
#         r2_reverse_score = sum(m.score for m in r2_reverse_matches) 

#         use_reverse_complement = False
#         while True:
#             if r1_score == max(r1_score, r1_reverse_score, r2_score, r2_reverse_score):
#                 r1_trimmed, r2_trimmed = r1_forward_trimmed_read, r2
#                 info1.is_rc = info2.is_rc = False
#                 break
#             if r2_score == max(r2_score, r1_reverse_score, r2_score, r2_reverse_score):
#                 r1_trimmed, r2_trimmed = r2_forward_trimmed_read, r1
#                 info1.is_rc = info2.is_rc = False
#                 self.reverse_complemented += 1
#                 use_reverse_complement = True
#                 break
#             if r1_reverse_score == max(r1_score, r1_reverse_score, r2_score, r2_reverse_score):
#                 r1_trimmed, r2_trimmed = r2, r1_reverse_trimmed_read_right
#                 info1.is_rc = info2.is_rc = True
#                 break
#             if r2_reverse_score == max(r1_score, r1_reverse_score, r2_score, r2_reverse_score):
#                 r1_trimmed, r2_trimmed = r2_reverse_trimmed_read_right, r1 
#                 info1.is_rc = info2.is_rc = True
#                 self.reverse_complemented += 1
#                 use_reverse_complement = True
#                 break
...
#         if r1_matches or r1_reverse_matches:
#             self.adapter_cutter1.with_adapters += 1  # type: ignore
#             for match in r1_matches:
#                 stats = self.adapter_cutter1.adapter_statistics[match.adapter]  # type: ignore
#                 stats.add_match(match)
#                 stats.reverse_complemented += bool(use_reverse_complement)
#             info1.matches.extend(r1_matches)  # TODO extend or overwrite?
#             info2.matches.extend(r1_matches)
#         if r2_matches:
#             self.adapter_cutter1.with_adapters += 1  # type: ignore
#             for match in r2_matches:
#                 stats = self.adapter_cutter1.adapter_statistics[match.adapter]  # type: ignore
#                 stats.add_match(match)
#                 stats.reverse_complemented += bool(use_reverse_complement)
#             info1.matches.extend(r2_matches)
#             info2.matches.extend(r2_matches)  # TODO extend or overwrite?
#         return r1_trimmed, r2_trimmed
# ##### Changing Block End